...
export type AVMPerformanceLogOutput = {
...
type AVMPerformanceLogs = {
  [tag: string]: AVMPerformanceLogEntry;
};
...
export class EVMPerformanceLogger {
  private opcodes!: AVMPerformanceLogs;
  private precompiles!: AVMPerformanceLogs;
  ...
  public getLogs() {
    ...
    function getLogsFor(obj: AVMPerformanceLogs) {
      const output: AVMPerformanceLogOutput[] = [];
      for (const key in obj) {
        const field = obj[key];
        const gasPerSecond = field.gasUsed / field.time;
        const entry: AVMPerformanceLogOutput = {
          calls: field.calls,
  ...
  private unpauseTimer(timer: Timer) {
    if (this.currentTimer !== undefined) {
